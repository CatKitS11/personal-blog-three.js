...
const SignUp = () => {
  ...
  // Auto-hide alert หลัง 3 วินาทีสำหรับ success
  useEffect(() => {
  ...
  const onSubmit = async (e) => {
    ...
    if (
      ...
      setAlert({
        ...
        type: "error",
        message:
          "Please fix the errors before submitting. Username or email may already be taken.",
      });
      return;
    }
    ...
    try {
      ...
      if (result?.error) {
        ...
        setAlert({
          ...
          type: "success",
          message:
            "Account created successfully! Redirecting to success page...",
        });
      }
    } catch (error) {
  ...
  const getStatusDisplay = (field) => {
    ...
    if (!status || status.status === undefined) return null;
      ...
      <main className="px-4 py-8">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mt-10 md:mt-16 rounded-xl border border-gray-200 bg-[#EFEEEB] p-6 md:p-10">
            <h1 className="text-4xl font-semibold text-center text-[#333333] mb-8">
              Sign up
            </h1>
            <form
              onSubmit={onSubmit}
              className="max-w-xl mx-auto space-y-6 flex flex-col gap-2 text-left"
            >
              <div>
                <label
                  htmlFor="fullName"
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={onChange}
                  className={`bg-white ${
                    availabilityStatus.username?.status === "error"
                      ? "border-red-500"
                      : availabilityStatus.username?.status === "success"
                      ? "border-green-500"
                      : ""
                  }`}
                  disabled={state.loading}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={onChange}
                  className={`bg-white ${
                    availabilityStatus.email?.status === "error"
                      ? "border-red-500"
                      : availabilityStatus.email?.status === "success"
                      ? "border-green-500"
                      : ""
                  }`}
                  disabled={state.loading}
              ...
              <div className="flex justify-center pt-4">
                <Button
                  type="submit"
                  className="py-[24px] px-12 bg-gray-900 text-white font-light rounded-full hover:bg-gray-800 transition-colors"
                  disabled={
                    state.loading ||
                    availabilityStatus.username?.available === false ||
                    availabilityStatus.email?.available === false ||
